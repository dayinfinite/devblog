...
class User(db.Model):
    ...
    followed = db.relationship('Follow',
                               foregin_keys=[Follow.follower_id],
    ...
    def generate_fake(count=100):
        ...
        for i in range(count):
            u = User(email=forgery_py.internet.email_address(),
                     ...
                     localtion=forgery_py.address.city(),
    ...
    def followed_posts(self):
        return Post.query.join(Follow, Follow.follower_id == Post.author_id).filter(Follow.follower_id == self.id)
    ...
    def generate_auth_token(self, expiration):
        ...
        return s.dumps({'id':self.id}).dcode('ascii')
...
class Post(db.Model):
    ...
    def on_changed_body(target, value, oldvalue, initiator):
        ...
        target.body_html = bleach.linkify(bleach.clean(
            markdown(value, output_fotmat='html'),
